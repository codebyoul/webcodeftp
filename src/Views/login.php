...
<head>
    ...
    <title>Login - <?= htmlspecialchars($app_name ?? 'WebFTP', ENT_QUOTES, 'UTF-8') ?></title>
...
<body class="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 min-h-screen flex items-center justify-center p-4 transition-colors duration-200">

    <div class="w-full max-w-md">
        <!-- Logo/Header -->
        <div class="text-center mb-8">
            <h1 class="text-4xl font-bold text-gray-900 dark:text-white mb-2"><?= htmlspecialchars($app_name ?? 'WebFTP', ENT_QUOTES, 'UTF-8') ?></h1>
            <p class="text-gray-600 dark:text-slate-400"><?= htmlspecialchars($translations['app_subtitle'] ?? 'Secure File Management Portal', ENT_QUOTES, 'UTF-8') ?></p>
        </div>

        <!-- Login Card -->
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow-2xl p-8 border border-gray-200 dark:border-slate-700">
            <h2 class="text-2xl font-semibold text-gray-900 dark:text-white mb-2"><?= htmlspecialchars($translations['login_title'] ?? 'Sign In', ENT_QUOTES, 'UTF-8') ?></h2>
            <p class="text-gray-600 dark:text-slate-400 text-sm mb-6"><?= htmlspecialchars($translations['login_subtitle'] ?? 'Enter your FTP credentials to access your files', ENT_QUOTES, 'UTF-8') ?></p>
            ...
            <form method="POST" action="/login" class="space-y-6">
                <!-- CSRF Token -->
                <input type="hidden" name="<?= htmlspecialchars($csrf_field_name, ENT_QUOTES, 'UTF-8') ?>" value="<?= htmlspecialchars($csrf_token, ENT_QUOTES, 'UTF-8') ?>">
            ...
            <div class="mt-6 pt-6 border-t border-gray-200 dark:border-slate-700">
                <div class="flex items-start text-xs text-gray-600 dark:text-slate-400">
                    ...
                    <p><?= htmlspecialchars($translations['security_notice'] ?? 'Your credentials are encrypted and secure.', ENT_QUOTES, 'UTF-8') ?></p>
                </div>
            </div>
        </div>
        ...
        <div class="text-center mt-6 text-gray-500 dark:text-slate-500 text-sm">
            <p>Powered by <?= htmlspecialchars($app_name ?? 'WebFTP', ENT_QUOTES, 'UTF-8') ?> - <?= htmlspecialchars($translations['footer_text'] ?? 'Secure File Management', ENT_QUOTES, 'UTF-8') ?></p>
        </div>
    </div>
